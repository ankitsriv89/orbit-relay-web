...
| `public/objects/` | **Object Encyclopedia** (object-profiles plan) | Descriptive knowledge base — mission, operator, bus, mass, status, each fact sourced. Filter table over `profiles` via `/api/objects`; per-object detail pages are a crawlable shell (`functions/objects/[norad]/index.js`, server-injected `<title>` + JSON-LD) hydrated by `object.js` from `/api/object/:norad`'s new `profile` key. No globe — `docs.css` + `tokens.css`, depth 1 so shared refs are root-absolute |
...
| `functions/api/{objects,profile/[norad],img/[[path]]}.js` + `_profiles.js` | Object-profile endpoints | Read side of the `orbit-profiles` D1 (`PROFILE_DB`). `_profiles.js#profileFor` never throws — PROFILE_DB unbound or row missing ⇒ `null`, so `/api/object/:norad` gains a null-safe `profile` key and never 500s; `requireProfileDb` 503s for `/api/profile/:norad` which IS the profile. `objects.js` mirrors `search.js`'s facet cascade. `img/[[path]].js` is an R2 passthrough validated against `profiles/<digits>/(primary|thumb).<ext>`. Tested in `workers/orbit-profiles/test/pages-api.test.mjs` |
...
| `workers/orbit-profiles/` | **Object-profile enrichment** (object-profiles plan) | Writes the second D1 (`orbit-profiles`, `PROFILE_DB`) — kept apart from `orbit-catalog`'s read budget. Four resumable stages (match · facts · prose · images), `workflow_dispatch` only, no cron. `src/sources.js` is the licence allowlist enforced at the write path; `src/validate.js` rejects any prose numeral absent from the facts (derived ones included); Tier 2 prose is deterministic and model-free, Tier 3 is Groq-gated by the validator. `scripts/env-node.mjs` re-exports orbit-ingest's tested D1/R2/AI shims |
| `d1/profiles.sql` | `orbit-profiles` D1 schema | Five tables: `sources` (the allowlist, an enforcement mechanism not a lookup), `profiles`, `profile_fields` (provenance sidecar, PK `(norad, field)`), `images`, `ingest_checkpoints`. Idempotent, re-runnable |
