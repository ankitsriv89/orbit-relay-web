...
| `functions/api/_orbit.js` + `_catalog.js` | Shared function helpers | `_orbit.js:17-19` is the `X-Data-Source` citation, legally required on every response |
| `functions/api/telemetry.js` | **Dead** | Reads `env.TELEMETRY_DB`, which is not bound in `wrangler.toml` — permanently in its accept-and-drop path. Leftover game analytics; plan 34 assumes deleting it with `d1/telemetry.sql` |
